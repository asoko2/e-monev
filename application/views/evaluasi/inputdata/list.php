                ...
                <?php foreach ($val as $keg): ?>
                    <tr style="background: #ffffff">
                        <?php if ($keg['kode_status'] == 1) : ?>
                            <td></td>
                        <?php else : ?>
                        <td><a data-toggle="tooltip" data-placement="right" title="" data-original-title="Hapus Kegiatan <?php echo $keg['nama_kegiatan'] ?>" class="btn btn-danger btn-xs s-tooltip" href="<?php echo site_url('evaluasi/dpa/hapus_kegiatan/' . $hash.'.'.$keg['kode_kegiatan'].'/'.base64_encode($hash)) ?>"><i class="fa fa-times"></i></a></td>
                        <?php endif ?>
