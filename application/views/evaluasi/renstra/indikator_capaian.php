...
<div class="page-header">
    <h1><?php _e($page_title . ' <b>' . $program . '</b>', 'Data Master')?></h1>
</div>
...
<div class="panel">
    <div id="select2container" class="panel-body">
        <div class="row">
            <div class="col-md-2">
                <a class="btn btn-success" href="<?php echo site_url('evaluasi/dpa/tambah_indikator_program/'.$kode) . '?__=' . base64_encode($program) ?>">Tambah</a> <a class="btn btn-warning" href="<?php echo site_url('evaluasi/dpa/index?hash='.$hash) ?>">Kembali</a>
            </div>
        </div>
    </div>
</div>
        ...
        <tbody>
            <?php foreach ($data as $v): ?>
                <tr class="text-bold" style="background: #ededed">
                    <td>
                        <a data-toggle="tooltip" data-placement="right" title="" data-original-title="Hapus Indikator Kegiatan <?php echo $v['indikator'] ?>" class="btn btn-danger btn-xs s-tooltip" href="<?php echo site_url('evaluasi/dpa/hapus_indikator_program/' . $v['pk'].'/'.base64_encode($v['pk'])) ?>"><i class="fa fa-times"></i></a>
                    </td>
