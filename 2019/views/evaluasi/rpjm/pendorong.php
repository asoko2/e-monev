    ...
    <table class="table table-bordered">
        ...
        <?php foreach ($pendorong as $key => $value): ?>
            <tr>
                <td><?= $key+1 ?></td>
                <td>
                    <a href="#" id="pendorong" data-type="textarea" data-pk="<?= $value->id ?>" data-url="<?= base_url('evaluasi/rpjm/update_pendorong') ?>" data-title="pendorong"><?= $value->pendorong ?></a>
                </td>
...
<script type="text/javascript">
    init.push(function () {
         ...
         $('button[name="hapus"]').click(function() {
            ...
            var id = $(this).data('id');
             Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        url: '<?= base_url('evaluasi/rpjm/hapus_pendorong') ?>',
                        type: 'Post',
                        data: {id: id},
                    })
        ...
        $('button[name="form_pendorong_simpan"]').click(function() {
            $.ajax({
                url: '<?= base_url('evaluasi/rpjm/add_pendorong') ?>',
                type: 'POST',
                data: {
