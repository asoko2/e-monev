    ...
    <table class="table table-bordered">
        <thead>
            <tr>
                ...
                <th rowspan="3">Jenis Indikator</th>
                <th rowspan="3">Data Capaian pada Awal Tahun Perencanaan</th>
                <th rowspan="3">Target pada Akhir Tahun Perencanaan</th>
                ...
                <?php foreach ($urusan as $key => $bidang): ?><!-- #BIDANG -->
                    ...
                    <?php foreach ($bidang as $key => $program): ?><!-- #PROGRAM -->
                        ...
                        <tr bgcolor="#eee">
                            ...
                            <td><?php make_editable($program[0], 'sasaran_program', ['pk'=>$key[2], 'val'=>'urai_sasaran_rpjm']) ?></td>
                            <td><?php _e($key[0]) ?></td>
                            <td colspan="5"> 
                                <?php _e($key[1]) ?> &nbsp;&nbsp; <a data-toggle="tooltip" data-placement="left"  data-pk="<?php echo $key[2] ?>" title="" data-original-title="Tambah indikator" class="btn btn-success btn-xs s-tooltip tambah-capaian" href="<?php echo site_url('evaluasi/rpjm/tambah_indikator_capaian/' . $key[2]) ?>"><i class="fa fa-plus"></i></a>
                            </td>
                            <td>
                                <?php make_editable($program[0], 'target_pagu_rpjm_1', ['pk'=>$key[2], 'val'=>'t_rpjm_1']) ?>
                            </td>
                             
                            <td>
                                <?php make_editable($program[0], 'target_pagu_rpjm_2', ['pk'=>$key[2], 'val'=>'t_rpjm_2']) ?>
                            </td>
                            
                            <td>
                                <?php make_editable($program[0], 'target_pagu_rpjm_3', ['pk'=>$key[2], 'val'=>'t_rpjm_3']) ?>
                            </td>
                            
                            <td>
                                <?php make_editable($program[0], 'target_pagu_rpjm_4', ['pk'=>$key[2], 'val'=>'t_rpjm_4']) ?>
                            </td>
                            
                            <td>
                                <?php make_editable($program[0], 'target_pagu_rpjm_5', ['pk'=>$key[2], 'val'=>'t_rpjm_5']) ?>
                            </td>
                        ...
                        <?php foreach ($capaian as $cap): ?>
                            <tr>
                                ...
                                <td>
                                    <?php make_editable($cap, 'jenis_indikator', ['pk'=>$cap['id'], 'val'=>'jenis_indikator']) ?>   
                                </td>
...
<script type="text/javascript">
    ...
    init.push(function () {
        ...
        $('.e_target_pagu_rpjm_<?php echo $i ?>').editable({
            type: 'text',
            mode: 'inline',
            name: 'target_pagu_t_rpjm_<?php echo $i ?>' ,
            url: '<?php echo site_url("bypass") ?>',
            success: function(response, newValue) {
                var res = $.parseJSON(response);
                if (res.status == false) 
