    ...
    <table class="table table-bordered">
        ...
        <?php foreach ($pendorong as $key => $value): ?>
            <tr>
                <td><?= $key+1 ?></td>
                <td>
                    <a href="#" id="pendorong" data-type="textarea" data-pk="<?= $value->id ?>" data-url="<?= base_url('evaluasi/rpjm/update_t_rkpd') ?>" data-title="tindak lanjut"><?= $value->tindak_lanjut ?></a>
                </td>
...
<script type="text/javascript">
    init.push(function () {
         ...
         $('button[name="hapus"]').click(function() {
            ...
            var id = $(this).data('id');
             Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        url: '<?= base_url('evaluasi/rpjm/hapus_t_rkpd') ?>',
                        type: 'Post',
                        data: {id: id},
                    })
        ...
        $('button[name="form_pendorong_simpan"]').click(function() {
            $.ajax({
                url: '<?= base_url('evaluasi/rpjm/add_t_rkpd') ?>',
                type: 'POST',
                data: {
