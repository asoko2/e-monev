...
<div class="page-header">
    <h1><?php _e("Tindak lanjut yang diperlukan dalam RKPD Selanjutnya")?></h1>
</div>
<div class="panel">
    <div id="select2container" class="panel-body">
        <div class="row">
             ...
             <div class="col-md-2">
                <a class="btn btn-success" href="#" data-toggle="modal" data-target="#modal-pendorong">Tindak Lanjut</a>
            </div>
        </div>
    </div>
</div>
    ...
    <table class="table table-bordered">
        ...
        <?php foreach ($pendorong as $key => $value): ?>
            <tr>
                <td><?= $key+1 ?></td>
                <td>
                    <a href="#" id="pendorong" data-type="textarea" data-pk="<?= $value->id ?>" data-url="<?= base_url('evaluasi/rpjm/update_t_rpjmd') ?>" data-title="tindak lanjut"><?= $value->tindak_lanjut ?></a>
                </td>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title">Tambah Tindak Lanjut RKPD selanjutnya</h4> </div>
...
<script type="text/javascript">
    init.push(function () {
         ...
         $('button[name="hapus"]').click(function() {
            ...
            var id = $(this).data('id');
             Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        url: '<?= base_url('evaluasi/rpjm/hapus_t_rpjmd') ?>',
                        type: 'Post',
                        data: {id: id},
                    })
        ...
        $('button[name="form_pendorong_simpan"]').click(function() {
            $.ajax({
                url: '<?= base_url('evaluasi/rpjm/add_t_rpjmd') ?>',
                type: 'POST',
                data: {
