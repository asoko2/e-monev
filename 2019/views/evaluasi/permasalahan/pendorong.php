    ...
    <div id="select2container" class="panel-body">
        <div class="row">
            ...
            <div class="col-md-2">
             ...
             <div class="col-md-2">
                <a class="btn btn-success" href="#" data-toggle="modal" data-target="#modal-pendorong">Tambah Item Pendorong</a>
            </div>
        </div>
    </div>
</div>
    ...
    <table class="table table-bordered">
        ...
        <?php foreach ($pendorong as $key => $value): ?>
            <tr>
                <td><?= $key+1 ?></td>
                <td>
                    <a href="#" id="pendorong" data-type="textarea" data-pk="<?= $value->id ?>" data-url="<?= base_url('evaluasi/permasalahan/update_pendorong') ?>" data-title="pendorong"><?= $value->pendorong ?></a>
                </td>
...
<script type="text/javascript">
    init.push(function () {
         ...
         $('button[name="hapus"]').click(function() {
            ...
            var id = $(this).data('id');
             Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        url: '<?= base_url('evaluasi/permasalahan/hapus_pendorong') ?>',
                        type: 'Post',
                        data: {id: id},
                    })
        ...
        $("#select_unit").on("change", function(e) {
            ...
            window.location.href = "<?php echo(site_url("evaluasi/permasalahan/pendorong?hash=")) ?>" + $(this).val() + '&tw=' + $('select[name="Triwulan"]').val();
        });


        $('select[name="Triwulan"]').on("change", function(e) {

            window.location.href = "<?php echo(site_url("evaluasi/permasalahan/pendorong?hash=")) ?>" + $("#select_unit").val() + '&tw=' + $('select[name="Triwulan"]').val();
        });

        $('button[name="form_pendorong_simpan"]').click(function() {
            $.ajax({
                url: '<?= base_url('evaluasi/permasalahan/add_pendorong') ?>',
                type: 'POST',
                data: {
                    Pendorong      : $('textarea[name="Pendorong"]').val(),
