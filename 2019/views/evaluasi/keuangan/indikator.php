                ...
                <?php foreach ($urusan as $key => $bidang): ?>
                    ...
                    <?php foreach ($bidang as $key => $program): ?>
                        ...
                        <tr bgcolor="#ededed">
                            ...
                            <td class="text-right">
                                <?php make_editable($capaian[0], 'target_capaian_renstra', [
                            ...
                            <?php for ($i=1; $i < 13; $i++) : ?>
                                <script type="text/javascript">
                                    init.push(function () {
                                        $('.e_target_capaian_b<?php echo $i ?>').editable({
                                            type: 'text',
                                            mode: 'inline',
                                            name: 'target_capaian_b<?php echo $i ?>',
                            ...
                            <td class="text-right">
                                <?php make_editable($capaian[0], 'target_capaian_b' .$i, [
                                    'pk' => $program[0]['pkp'] . '.' . $capaian[0]['urut'],
                                    'val' => 'target_b' .$i
                                ]) ?>
                            </td>
                        ...
                        <?php foreach ($program as $kegiatan): ?>
                            ...
                            <tr bgcolor="#ffffff">
                                ...
                                <td class="text-right">
                                    <?php make_editable($keluaran[0], 'target_keluaran_renstra', [
                                ...
                                <?php for ($i=1; $i < 13; $i++) : ?>
                                    <script type="text/javascript">
                                        init.push(function () {
                                            $('.e_target_keluaran_b<?php echo $i ?>').editable({
                                                type: 'text',
                                                mode: 'inline',
                                                name: 'target_keluaran_b<?php echo $i ?>',
                                                url: '<?php echo site_url("bypass") ?>',
                                                success: function(response, newValue) {
                                                    var res = $.parseJSON(response);
                                                    if (res.status == false) 
                                ...
                                <td class="text-right">
                                    <?php make_editable($keluaran[0], 'target_keluaran_b' .$i, [
                                        'pk' => $kegiatan['pk'] . '.' . $keluaran[0]['urut'],
                                        'val' => 'target_b' .$i
                                    ]) ?>
                                </td>
...
<script type="text/javascript">
    init.push(function () {
        ...
        $('#select_unit').on("change", function(e) {
            console.log(e.val)
            window.location.href = '<?php echo(site_url("evaluasi/keuangan/indikator?hash=")) ?>' + e.val
        });

        $('.e_target_keluaran_renstra').editable({
            type: 'text',
            mode: 'inline',
            name: 'target_keluaran_renstra',
        ...
        $('.e_target_capaian_renstra').editable({
            type: 'text',
            mode: 'inline',
            name: 'target_capaian_renstra',
