                    ...
                    <?php foreach ($bidang as $key => $program): ?><!-- #PROGRAM -->
                        ...
                        <?php foreach ($capaian as $cap): ?>
                            <tr>
                                ...
                                <td class="text-right"><?php make_editable($cap, 'realisasi_capaian_0', ['pk'=>$cap['id'], 'val'=>'r_renstra_'.($tahun_aktif-2019)]) ?></td>
...
<script type="text/javascript">
    init.push(function () {
        ...
        $('.e_realisasi_capaian_0').editable({
            type: 'text',
            mode: 'inline',
            name: 'realisasi_capaian_r_renstra_<?= ($tahun_aktif-2019) ?>' ,
            url: '<?php echo site_url("bypass") ?>',
            success: function(response, newValue) {
                var res = $.parseJSON(response);
                if (res.status == false) 
