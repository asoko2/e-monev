...
<table border="0.5pt" style="width:310mm" >
    <thead>
        <tr>
            <th colspan="20">FORMULIR 1. EVALUASI HASIL RENJA OPD KABUPATEN BOJONEGORO</th>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <th rowspan="2" <?php _col(5) ?>>Target RPJMD Akhir Periode <?php _e($list_renstra[1]) ?> - <?php _e($list_renstra[5]) ?></th>
            <th rowspan="2" <?php _col(5) ?>>Realisasi capaian kinerja RPJMD OPD sampai dengan Tahun <?php _e($list_renstra[$tahunkey]) ?></th>
            <th rowspan="2" <?php _col(5) ?>>Target Kinerja OPD Tahun Berjalan (Tahun <?php _e($list_renstra[$tahunkey]) ?>) yang dievaluasi</th>
            <th colspan="4" <?php _col(16) ?>>Realisasi Kinerja Pada Triwulan</th>
            <th rowspan="2" <?php _col(5) ?>>Realisasi Capaian Kinerja dan Anggaran RKPD OPD <?php _e($list_renstra[$tahunkey]) ?> yang dievaluasi</th>
            ...
            <th rowspan="2" <?php _col(5) ?>>Realisasi Kinerja RPJMD OPD s/d Tahun <?php _e($list_renstra[$tahunkey]) ?></th>
            <th rowspan="2" <?php _col(2) ?>>Tingkat Capaian Kinerja RPJMD OPD s/d Tahun <?php _e($list_renstra[$tahunkey]) ?> (%)</th>
            <th rowspan="2" <?php _col(2) ?>>OPD Penanggung Jawab</th>
    ...
    <tbody>
       <?php if (count($data) > 0): ?>
            ...
            <?php foreach ($data as $key => $urusan): ?><!-- #URUSAN -->
               ...
               <?php foreach ($urusan as $key => $bidang): ?><!-- #BIDANG -->
                   ...
                   <tr bgcolor="#dedede">
                       <td></td>
                       <td></td>
                       <td><?php echo $kd_urusan.'.'.$kd_bidang ?></td>
                       <td colspan="15"><?php _e($key[1]) ?></td>
                   </tr>
                   <?php foreach ($bidang as $key => $program): ?><!-- #PROGRAM -->
                       ...
                       <tr bgcolor="#eee">
                           ...
                           <td><?php echo $kd_urusan.'.'.$kd_bidang.'.'.$kd_program  ?></td>
                           <td colspan="15"><?php _e($key[1]) ?></td>
                       </tr>
                       ...
                       <?php foreach ($program as $kegiatan): ?><!-- #KEGIATAN -->
                           ...
                           <tr>
                               <td></td>
                               <td></td>
                               <td><?php echo $kd_urusan.'.'.$kd_bidang.'.'.$kd_program.'.'.$kegiatan["kode_kegiatan"]?></td>
                       ...
                       <tr style="font-weight: 700; text-align: right">
                           <td colspan="13">Rata-rata Capaian Anggaran</td>
                       ...
                       <tr style="font-weight: 700; text-align: right">
                           <td colspan="13">Predikat Capaian Anggaran</td>
                       ...
                       <tr style="font-weight: 700; text-align: right">
                           <td colspan="13">Rata-rata Capaian Kinerja</td>
                       ...
                       <tr style="font-weight: 700; text-align: right">
                           <td colspan="13">Predikat Capaian Kinerja</td>
           ...
           <tr style="font-weight: 700; text-align: right">
               <td colspan="7">JUMLAH ANGGARAN DAN REALISASI DARI SELURUH PROGRAM</td>
           ...
           <?php
            $total_pagu = array_sum($f_total_7);
            $total_realisasi = array_sum($f_total_12);
            $f_rata_anggaran = ($total_realisasi / $total_pagu) *100;
            $f_rata_anggaran2 = ($total_realisasi / $total_pagu) *100;
           ?>
           <tr style="font-weight: 700; text-align: right">
               <td colspan="13">Total Rata-rata Capaian Anggaran Dari Seluruh Program ( 1 s/d <?php echo $f_jml_program?>)</td>
               <td><?php echo persen(($f_rata_anggaran)); ?></td>
               <td></td>
               <td><?php echo persen(($f_rata_anggaran2)); ?></td>
               <td></td>
               <td></td>
           </tr>
           <tr style="font-weight: 700; text-align: right">
               <td colspan="13">Predikat Capaian Anggaran Seluruh Program ( 1 s/d <?php echo $f_jml_program?>)</td>
               <td><?php echo predikat(($f_rata_anggaran)); ?></td>
               <td></td>
               <td><?php echo predikat(($f_rata_anggaran2)); ?></td>
               <td></td>
               <td></td>
           </tr>
           <tr style="font-weight: 700; text-align: right">
               <td colspan="13">Total Rata-rata Capaian Kinerja Seluruh Program ( 1 s/d <?php echo $f_jml_program?>)</td>
           ...
           <tr style="font-weight: 700; text-align: right">
               <td colspan="13">Predikat Capaian Kinerja Seluruh Program ( 1 s/d <?php echo $f_jml_program?>)</td>
            ...
            <tr>
             <td colspan="18">
               Faktor Pendorong Keberhasilan Kinerja : <br>
                <ul>
               <?php foreach ($permasalahan['pendorong'] as $key => $value): ?>
            ...
            <tr>
              <td colspan="18">
               Faktor Penghambat Pencapaian Kinerja : <br>
                <ul>
               <?php foreach ($permasalahan['penghambat'] as $key => $value): ?>
            ...
            <tr>
              <td colspan="18">
               Tindak lanjut yang diperlukan dalam triwulan berikutnya : <br>
                <ul>
               <?php foreach ($permasalahan['next_tw'] as $key => $value): ?>
            ...
            <tr>
              <td colspan="18">
               Tindak lanjut yang diperlukan dalam Renja PD Tahun Berikutnya : <br>
                <ul>
               <?php foreach ($permasalahan['next_rkpd'] as $key => $value): ?>
